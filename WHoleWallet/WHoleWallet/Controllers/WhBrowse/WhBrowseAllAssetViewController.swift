...
/*******************************************************************************
        ...
        WhBrowseAllAssetViewController.swift
        WHoleWallet
...
class WhBrowseAllAssetViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, IndicatorInfoProvider, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        if emptyView == nil {
            ...
            container.snp.makeConstraints { (make) in
                make.top.equalTo(WhBrowseAllAssetViewController.barHight)
...
//        refreshControl.backgroundColor = UIColor(hex: 0x0C66FF)
        ...
        //searchbar
        let searchBar = UISearchBar(frame: CGRect(x: 0, y: 0, width: Int(screenWidth()), height: WhBrowseAllAssetViewController.barHight))
    ...
    func fetch(direction: WhPullDirection, serach: String?) {
        ...
        if let searchKey = serach {
            if searchKey.count > 0 {
                path = "property/list?pageSize=\(pageSize)&pageNo=\(pageNo)&keyword=\(searchKey)"
            }else{
                path = "property/list?pageSize=\(pageSize)&pageNo=\(pageNo)"
            }
        }else {
            path = "property/list?pageSize=\(pageSize)&pageNo=\(pageNo)"
        }
