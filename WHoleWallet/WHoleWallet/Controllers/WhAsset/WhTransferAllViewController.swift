...
/*******************************************************************************
        ...
        WhTransferAllViewController.swift
        WHoleWallet
...
class WhTransferAllViewController: UIViewController, UITextFieldDelegate {
    ...
    @objc func sureAction()  {
        ...
        guard let fee = feeTF.text,  let to = toTF.text else {
        ...
        let parameters = ["transaction_version": "0", "fee":fee, "transaction_from": wallet.cashAddr,"transaction_to": to, "ecosystem": "1"]
        WhHTTPRequestHandler.unsignedOperate(reqCode: 4, parameters: parameters) { (result) in
            if result.count > 0 {
