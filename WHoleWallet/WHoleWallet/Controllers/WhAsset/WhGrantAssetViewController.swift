...
/*******************************************************************************
        ...
        WhGrantAssetViewController.swift
        WHoleWallet
...
class WhGrantAssetViewController: UIViewController, UITextFieldDelegate {
    ...
    func configView() {
        ...
        feeRate.snp.makeConstraints { (make) in
            make.left.equalTo(grantCount.snp.left)
            make.right.equalTo(grantCount.snp.right)
            make.top.equalTo(grantCount.snp.bottom).offset(15)
            make.height.equalTo(grantCount.snp.height)
        }
    ...
    @objc func sureAction()  {
        ...
        let parameters = ["transaction_version": "0", "fee":fee, "transaction_from": wallet.cashAddr, "amount": amount, "currency_identifier": pID]
        WhHTTPRequestHandler.unsignedOperate(reqCode: 55, parameters: parameters) { (result) in
            if result.count > 0 {
